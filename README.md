...
GuardPay scores card payments and flags fraudulent transactions the moment they happen, instead of finding fraud later in a next-day report. It runs entirely as streaming SQL on Confluent Cloud. Connectors bring in a live stream of card transactions plus reference data for cardholders and merchant risk (generated from sample schemas for this demo). Using Apache Flink, I created tables that join each transaction to the cardholder and merchant risk, keep a rolling per-card profile, and assign a fraud risk score using both rules (velocity, impossible travel, high-risk merchant, large amounts) and a built-in anomaly check on spend. A sink connector pushes the alerts out to a downstream system. Every stream is governed by Schema Registry and the whole flow is visible in Stream Lineage.
